...
public class Type2 
{
	...
	public int createHeader(PdfWriter w,int verticalindex,String string,Document d) throws FileNotFoundException, DocumentException
	{
		...
		PdfWriter writer=w;
		Phrase h=new Phrase();
	    FontFactory.register("c:/windows/fonts/kokilabi.ttf", "kokilabi");
	    Font font=FontFactory.getFont("kokilabi", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,28);
	...
	{
		PdfContentByte canvas = writer.getDirectContentUnder();
		LineSeparator UNDERLINE1 = new LineSeparator(01,w,BaseColor.BLACK, Element.ALIGN_CENTER,pos);
		doc.add(UNDERLINE1);
	}
	public int normalText(PdfWriter writer,int verticalindex,String string) throws DocumentException
	{
		int normalspace=20;
		Phrase b=new Phrase();
		FontFactory.register("c:/windows/fonts/kokilab.ttf", "kokilab");
	    Font font=FontFactory.getFont("kokilab", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,12);
		PdfContentByte canvas = writer.getDirectContentUnder();
		ColumnText ct=new ColumnText(writer.getDirectContent());
	...
	public int createSubHeader(PdfWriter writer,int verticalindex,String string) throws DocumentException
	{
		...
		Phrase b=new Phrase();
		FontFactory.register("c:/windows/fonts/kokilabi.ttf", "kokilabi");
	    Font font=FontFactory.getFont("kokilabi", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,20);
		PdfContentByte canvas = writer.getDirectContentUnder();
		ColumnText ct=new ColumnText(writer.getDirectContent());
		b.add(new Paragraph(string,font));
		ct.setAlignment(Element.ALIGN_CENTER);
	...
	public int paragraphText(PdfWriter writer,int verticalindex,String string) throws DocumentException
	{
		...
		FontFactory.register("c:/windows/fonts/kokilab.ttf", "kokilab");
		   Font font=FontFactory.getFont("kokilab", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,13);
			PdfContentByte canvas = writer.getDirectContentUnder();
			ColumnText ct=new ColumnText(writer.getDirectContent());
